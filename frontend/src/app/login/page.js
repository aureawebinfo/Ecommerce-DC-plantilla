...
export default function Login() {
  ...
  const handleSubmit = async (e) => { //funcion de cuando el usuario hace click en ingresar
    e.preventDefault()
    setLoading(true)
    setError('')

    const result = await login(email, password) //Es la que envía el email y password a Django. 
      ...
      <header className="fixed top-2 md:top-4 left-0 right-0 z-50 px-2 md:px-4">
        <div className="container mx-auto bg-[#009045] text-white rounded-full shadow-2xl py-3 px-4 md:px-8 flex justify-between items-center border-b-[4px] border-[#007a3a]">
          ...
          <div className="flex items-center gap-4 md:gap-6 shrink-0">
            <Link href="/registro" className="hidden md:block bg-yellow-400 text-[#009045] px-5 py-2 rounded-full font-black hover:bg-white hover:scale-105 shadow-md transition-all">
              Regístrate
            </Link>
      ...
      <main className="container mx-auto px-4 py-8 -mt-16 relative z-20">
        <div className="max-w-md mx-auto animate-slide-up">
          ...
          <div className="bg-white rounded-[2.5rem] shadow-2xl p-8 md:p-10 border border-gray-100 relative overflow-hidden">
            ...
            {error && (
              <div className="bg-red-50 border-l-4 border-red-500 text-red-700 p-4 rounded-r-xl mb-6 animate-shake font-medium text-sm">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-[#009045] hover:bg-[#007a3a] text-white py-4 rounded-full font-black text-lg tracking-wide transition-all duration-300 transform hover:-translate-y-1 shadow-lg hover:shadow-xl mt-2 disabled:opacity-70 disabled:cursor-not-allowed"
              >
            ...
            <div className="text-center mt-8 pt-6 border-t border-gray-100">
              <p className="text-gray-600 font-medium mb-4">
                ¿No tienes cuenta?{' '}
                <Link href="/registro" className="text-[#009045] font-black hover:text-yellow-500 transition-colors underline decoration-2">
                  Regístrate gratis
                </Link>
              </p>
            </div>

            {/* Datos de prueba estilo nota adhesiva */}
            <div className="mt-4 p-4 bg-yellow-50 rounded-xl border border-yellow-200 transform rotate-1 hover:rotate-0 transition-transform duration-300">
              <p className="text-xs text-yellow-800 text-center font-medium">
                <strong>💡 Tip:</strong> Usa cualquier email para probar.
              </p>
            </div>
        ...
        <div className="container mx-auto px-4 pb-16 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-12 items-start">
            <div className="lg:col-span-1 flex justify-center lg:justify-start">
              <div className="text-[100px] leading-none drop-shadow-xl cursor-pointer hover:scale-110 transition-transform">
                🏪
              </div>
            </div>
